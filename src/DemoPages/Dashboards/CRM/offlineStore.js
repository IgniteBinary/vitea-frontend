...
class StoresDashBoard extends Component {
  ...
  editStore = (_id) => async () => {
    const store = await this.props.Stores.stores.find((x) => x._id === _id);
  ...
  render() {
    ...
    const offlineStore = data.filter((store) => store.store_type && store.store_type === 'offline');
        ...
        >
          ...
          >
            <TabPane tab='Stores' key='1'>
              {offlineStore.length> 0 && <StoresTable data={offlineStore} editStore={this.editStore} />}
              {offlineStore.length === 0 && <CardTitle className='text-center'>
                You currently do not have any offline stores{' '}
              </CardTitle>}
            </TabPane>
          </Tabs>
        </CSSTransitionGroup>
